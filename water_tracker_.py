...
class WaterTracker:
    ...
    def init_database(self):
        ...
        # Создание таблицы для записи потребления воды
        self.cursor.execute('''
    ...
    def create_main_tab(self):
        """Создание основной вкладки трекера"""
        # Заголовок
    ...
    def add_custom_water(self):
        """Добавление кастомного количества воды"""
        try:
    ...
    def get_today_intake(self):
        """Получение потребления воды за сегодня"""
        today = datetime.date.today().strftime("%Y-%m-%d")
    ...
    def update_history(self):
        ...
        if records:
            for record_id, amount, timestamp in records:
                time_str = timestamp.split()[1][:5]  # Только время HH:MM
                display_text = f"{time_str} - {amount} мл"
                self.history_listbox.insert(tk.END, display_text)
                # Сохраняем ID записи в качестве данных
    ...
    def clear_today_history(self):
        """Очистка всей истории за сегодня"""
    ...
    def show_telegram_help(self):
        """Показ справки по настройке Telegram"""
...
❗ Важно: Chat ID может быть отрицательным числом!"""
    ...
    def update_calendar(self):
        ...
        for week_num, week in enumerate(cal):
            for day_num, day in enumerate(week):
                ...
                # Цвет в зависимости от выполнения цели
                if intake >= self.daily_goal:
                    color = '#90EE90'  # Светло-зеленый
                elif intake > 0:
                    color = '#FFE4B5'  # Бежевый
                else:
                    color = '#FFB6C1'  # Светло-розовый (пропуск)
    ...
    def update_stats(self):
        ...
        # Подсчет дней с достигнутой целью
        self.cursor.execute('''
            SELECT COUNT(*) FROM (
    ...
    def start_notification_thread(self):
            ...
            while True:
                if self.notifications_enabled or self.telegram_enabled:
                    try:
                        ...
                        if remaining > 0:
                            message = f"💧 Пора выпить воды!\n\nОсталось до цели: {remaining} мл\nВыпито сегодня: {today_intake} мл"
                        ...
                        # Поздравление при достижении цели
                            ...
                            if not os.path.exists(congratulation_file):
                                congratulation_msg = f"🎉 Поздравляем!\n\nВы достигли дневной цели: {self.daily_goal} мл\nВыпито: {today_intake} мл"
                                ...
                                # Создаем флаг, чтобы не спамить поздравлениями
                ...
                time.sleep(self.reminder_interval * 60)  # Конвертация в секунды
        
        notification_thread = threading.Thread(target=notification_worker, daemon=True)
        notification_thread.start()
        
        # Очистка старых флагов поздравлений (удаляем файлы старше 1 дня)
        def cleanup_congratulation_flags():
            try:
                ...
                print(f"Ошибка очистки флагов: {e}")
